...
- Code blocks use Chroma classes (`noClasses = false`) with the github-dark style in `assets/css/syntax.css`, generated by `hugo gen chromastyles --style=github-dark`. The image render hook optimizes archived blog images without changing their original URLs or Markdown.
...
- The site is dark in every system theme (`color-scheme: dark`). Tokens live in `@theme` in `assets/css/site.css`: `bg-paper` is the canvas, `bg-surface` a tinted section, `text-accent` the single warm accent. Major containers are bezels: `shell` (the outer tray) wrapping `core` (the lit inner surface). Buttons with a trailing arrow use `island-solid` or `island-ghost`, which put the arrow in its own circle. A fixed `.grain` layer adds film grain, and blocks marked `data-reveal` rise in on scroll via `assets/js/reveal.ts` (skipped for reduced motion, and never hidden without JavaScript). Archived articles use `prose-invert`.
