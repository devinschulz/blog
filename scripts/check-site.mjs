...
for (const file of files) {
  ...
  if (file !== '404.html') {
    ...
    assert(image.startsWith(`${domain}/`), `${file}: social image must be absolute on ${domain}, got ${image}`);
...
const signatures = ['index.html', 'work/cape/index.html', 'work/invision/index.html', 'work/warranties/index.html'].map(file => cards.get(file));
assert(signatures.every(Boolean), 'Every signature page needs a sharing card');
assert.equal(new Set(signatures).size, signatures.length, `Signature pages must not share a card: ${signatures.join(', ')}`);
for (const [file, card] of cards) {
  if (file.startsWith('blog/')) assert.equal(card, 'writing', `${file}: writing shares one card, got ${card}`);
}
...
console.log(`Checked ${files.length} HTML pages: metadata, JSON-LD, agent discovery links, local links/assets, blog images, ${new Set(cards.values()).size} sharing cards, ${sitemapLocs.length} sitemap URLs, ${llmsLinks.length} llms.txt entries, and ${redirects.length} redirect mappings.`);
