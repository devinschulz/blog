...
- Every WebGL surface shares one harness in `assets/js/webgl-surface.ts`, so all of them pause offscreen and in hidden tabs, hold a still frame for reduced motion, hide in forced colors, and recover from a lost context. All are `aria-hidden` decoration. The home page carries two: the hero and the states lattice; the 404 page carries a scattered lattice.
- Each surface degrades on its own terms. The hero falls back to its SVG. Selected work uses real screenshots with alt text, so it never depended on a renderer. The states lattice has no static counterpart, so it leaves the layout rather than reserving empty space.
