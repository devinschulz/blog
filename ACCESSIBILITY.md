...
- Every WebGL surface shares one harness in `assets/js/webgl-surface.js`, so all of them pause offscreen and in hidden tabs, hold a still frame for reduced motion, hide in forced colors, and recover from a lost context. All are `aria-hidden` decoration. The home page carries six: the hero, the states lattice, three project tiles, and the Current chapter backdrop.
- Each surface degrades on its own terms. The hero falls back to its SVG and the project tiles to the CSS illustrations they were built from. The states lattice has no static counterpart, so it leaves the layout rather than reserving empty space.
- The Current chapter backdrop sits behind body copy on a `#5146df` panel where the note text starts at only 4.67:1. Its material is unlit and every tile color is darker than the panel, so compositing can only darken and the copy keeps exactly the contrast it has on the flat background — verified by sampling every interior pixel of the rendered panel. axe cannot check this: it reads the CSS background, not canvas pixels. Keep that invariant if you change the artwork.
