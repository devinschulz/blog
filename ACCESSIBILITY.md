...
`check:a11y` builds the production site and starts an isolated local test server. It runs axe WCAG A/AA checks (including available WCAG 2.2 rules) on every generated HTML page at 1440px and 320px, with the system theme set to light and to dark (the site itself is always dark). It also checks text-spacing overrides, heading clipping, keyboard navigation, filters, the demo workflow, motion preferences, GIF controls, forced-color focus indicators, and no-JavaScript fallbacks.
