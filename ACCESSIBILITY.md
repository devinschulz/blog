...
- Every WebGL surface shares one harness in `assets/js/webgl-surface.js`, so all of them pause offscreen and in hidden tabs, hold a still frame for reduced motion, hide in forced colors, and recover from a lost context. The states lattice below the Perspective headline is decorative: it is `aria-hidden`, and it leaves the layout entirely when there is no renderer or no JavaScript rather than reserving empty space.
